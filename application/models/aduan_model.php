...
class Aduan_model extends CI_Model  {
    ...
    function get_all()
    {
        //$this->log_model->set_log("Melihat Aduan");
    ...
    function get_nomor_hp($role = 1, $departemen = null, $id_aduan = null)
    {
        ...
        if($departemen == null)
        {
            $data = $this->db->query('select no_hp_petugas from petugas where role = '.$role.' and no_hp_petugas is not null')->result();
        }
        else if($departemen != null)
        {
			$data = $this->db->query('select no_hp_petugas from petugas where role = '.$role.' and departemen='.$departemen.' and no_hp_petugas is not null')->result();
        }
