...
/*
    public function get_konten($url)
    {
        ...
        foreach ($html->find('li[class=ptb15] div[class=mr140]') as $element) {
           ...
           // $konten["url"] = $element->href;

            array_push($data, $konten);
        }
        echo json_encode($data);
    }
*/
    ...
    {
		//$text_sms = "ghaha Terima Kasih atas ";
		//$balasan_sms = array();
		//$balasan_sms['DestinationNumber'] = "085649073300";
		//$balasan_sms['TextDecoded'] = " JOZZZ Terima Kasih atas ";
            ...
            if ($sms_valid) {
                ...
                if ($this->sms_model->sms_not_processed($value['ID'])) {
                    ...
                    if ($nomor_aduan = $this->aduan_model->add($insert_data)) {
                        ...
                        $text_sms = "Terima Kasih atas Aduan Anda, Nomor Aduan anda adalah $nomor_aduan_info. Silakan cek aduan anda di alamat http://surga.kedirikota.go.id";
                        $balasan_sms = array();
    ...
    function cek_notif()
    {
        ...
        foreach ($sPesan as $key => $value) {
            //var_dump($sPesan);
            $pesanW .= $key .":".$value["count"]."\r\n";
            $pesanD = "SURGA : Terdapat " .$value["count"]. " aduan belum terjawab pada Departemen anda ($key)";
            if($value["no_hp"] != null){

                if($this->pengaturan_model->get_value(1) == 0)
                {
	   ...
	   //$no_hp_walikota = "085645272715";
        
        $all_hp = $this->pengaturan_model->get_all_notifikasi();

        foreach ($all_hp as $key => $value) {

            $this->kirim_pesan($pesanW, $value['no_hp_walikota']);
            $this->linkSMS($value['no_hp_walikota'], $pesanW);
    ...
    function linkSMS($numb,$sms){
		...
		$balasan_sms = array();
		$balasan_sms['DestinationNumber'] = $numb;
		$balasan_sms['TextDecoded'] = $sms;
		$this->sms_model->send_sms($balasan_sms);
