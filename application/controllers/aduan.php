...
class Aduan extends CI_Controller {
	...
	public function update_info($nomor_aduan = 0)
	{
		if ($this->input->post()) {
			...
			if (isset($_POST["diteruskan"])) {
				...
				$data['info'] = '4';
				//echo "diteruskan";
				$sms_text = "Telah masuk 1 aduan baru ke departemen Anda silahkan tindak lanjuti aduan tersebut ke website ".base_url('login');
				$nomor_hp = $this->aduan_model->get_nomor_hp(3,$data['departemen']); //role 3 = staf,departemen post data array
	...
	public function sukses()
	{
		...
		$sms_text = "POOL : Telah masuk 1 aduan baru silahkan tindak lanjuti aduan tersebut ke website ".base_url('login');
		$nomor_hp = $this->aduan_model->get_nomor_hp(1,1); //1=role, departemen id 1 = humas
		foreach($nomor_hp as $nomor)
		{
			$balasan_sms = array();
			$balasan_sms['DestinationNumber'] = $nomor->no_hp_petugas;
